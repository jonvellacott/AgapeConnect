...
Namespace DotNetNuke.Modules.AgapePortal
    Partial Class CasAuth
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            Service = TabController.CurrentPage.FullUrl
            If Request.QueryString("mode") = "proxy" Then
                Service = "https://www.globalopsccci.org/gma41demo15/index.php"


                Dim service2 = Server.UrlEncode("https://www.globalopsccci.org/gma41demo15/?q=en/gmaservices&destination=gmaservices")
                Dim mc As New MobileCAS()
                Dim resp = mc.AuthenticateWithTheKey("markgriffen@bellsouth.net", "bubba2011", service2)

                'Service = "https://tntdataserver.eu/dataserver/mat/"
                '    Dim objKey As New KeyUser.KeyAuthentication("markgriffen@bellsouth.net", "bubba2011", service2)
                
                Response.Redirect(Service & "?q=gmaservices&ticket=" & resp.ProxyTicket)

                'Response.Redirect(Service & "dataquery/DataQueryService2.asmx/Auth_Login?ServiceName=" & Service & "&ServiceTicket=" & objKey.ProxyTicket & "&AutoRegister=false")



                ' Response.Redirect(NavigateURL() & "?ticket=" & objKey.ProxyTicket)


                ' Response.Write(objKey.ProxyTicket)
                ...
                Dim PGTIOU = UserInfo.Profile.GetPropertyValue("GCXPGTIOU")
                Dim p As New theKeyProxyTicket.PGTCallBackSoapClient()
                Dim PGT = p.RetrievePGTCallback("CASAUTH", "thecatsaysmeow3", PGTIOU)
                Dim PT = KeyUser.KeyAuthentication.GetProxyTicketFromCAS(CASHOST, Service, PGT)
                Response.Write(PT)
                Return
        ...
        Public Sub StaffLogin()
            ...
            If Request.QueryString("proxy") = "true" Then
                validateurl = CASHOST + "proxyValidate?" & "ticket=" & tkt & "&" & "service=" & Service
            End If
